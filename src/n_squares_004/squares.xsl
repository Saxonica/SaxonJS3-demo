...
<xsl:template name="xsl:initial-template">
  <doc>
    <xsl:text>Using a function that takes sequence and returns an array, the squares of </xsl:text>
    <xsl:value-of select="$numbers"/>
    <xsl:text> = </xsl:text>
    <xsl:value-of select="ex:squares-sequence-array($numbers)" separator=", "/>
  </doc>
</xsl:template>
