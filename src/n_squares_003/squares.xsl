...
<xsl:template name="xsl:initial-template">
  <doc>
    <xsl:text>Using a function that takes an array and returns a sequence, the squares of </xsl:text>
    <xsl:value-of select="$numbers"/>
    <xsl:text> = </xsl:text>
    <xsl:value-of select="ex:squares-array-sequence($numbers)" separator=", "/>
  </doc>
</xsl:template>
